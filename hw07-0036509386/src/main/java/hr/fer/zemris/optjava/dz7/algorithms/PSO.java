...
public class PSO {
    ...
    public PSO(FFANN network, int populationSize, double vMin, double vMax, double xMin, double xMax,
               double wMin, double wMax, double c1, double c2, int maxIter, double minError, Random rand) {
    ...
    public double[] run() {
        ...
        for (int i = 0; i < maxIter; i++) {
            ...
            double[] localFunction = new double[solutionSize];
            ...
            for (int j = 0; j < populationSize; j++) {
                for (int k = 0; k < dimension; k++) {
                    velocity[j][k] = w * velocity[j][k] + c1 * rand.nextDouble() * (personalBest[j][k] - values[j][k])
                            + c2 * rand.nextDouble() * (globalBest[k] - values[j][k]);
